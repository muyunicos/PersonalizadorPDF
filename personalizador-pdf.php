...
class Personalizador_PDF_Plugin
{
    ...
    public function enqueue_assets($hook)
    {
        ...
        wp_localize_script('personalizador-pdf', 'PersonalizadorPDF', [
            ...
            'imagenesBase' => $this->pmu_uploads()->url_ambito('img'),
        ]);
    }
    ...
    private function guardar_imagen($archivo, $letra, $origen, $ext)
    {
        ...
        $this->miniatura_grupo_textmuy_unlink($nombre, $letra); // nueva miniatura ThumbEngine (pmu/img/)
    ...
    /** Borra la miniatura ThumbEngine de un grupo de PDF (pmu/img/{pdf}-{letra}.webp). */
    private function miniatura_grupo_textmuy_unlink($pdf, $letra)
    {
        $dir = $this->pmu_uploads()->dir_ambito('img');
        @unlink($dir . DIRECTORY_SEPARATOR . strtolower($pdf) . '-' . $letra . '.webp');
    }
