...
## Conciliacion 004 vs 007 (norma ex-008 preservada en `constitution` §IV)

> ESTADO ex-008: la norma de conciliacion vigente esta preservada en la
> constitucion (era el spec 008, eliminado tras preservarse; NO era feature
> implementable directa; normaba los specs 004 y 007): `analisis.json`+`config.json`
> separados y `tmp/sesion-{sid}/` como unidad de render/carrito con preview
> obligatoria (`draft-{uuid}` -> `cart_item_key`). La implementacion futura sigue
> esa norma; lo de abajo es el historial.
