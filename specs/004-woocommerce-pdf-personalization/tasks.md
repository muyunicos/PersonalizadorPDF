...
- [X] T015 [US2] Habilitacion del carrito (server): `carrito_validar` (`woocommerce_add_to_cart_validation`: draft vigente del mismo PDF con `preview_estado` ok/omisible, aviso Woo si falta), `carrito_agregar` (`woocommerce_add_cart_item_data`: meta `pmu_sid`/`pmu_item_key`/`unique_key` UUID v4, evita fusion de lineas), `carrito_promover` (`woocommerce_add_to_cart`: `PMU_Sesion::promover()` al `cart_item_key` real, fail-safe), `carrito_mostrar` (`woocommerce_get_item_data`: etiquetas `cliente` con titulo del campo), `carrito_quitar` (`woocommerce_remove_cart_item`: `borrar_item()`) y `cantidad_fija` (min/max = 1 en productos vinculados). El congelado de webp sigue en T014 (cliente). Test: fase `carrito`. PENDIENTE (T014+): intercept del submit en `assets/tienda.js` y envio del POST con sid/item_key/unique_key.
