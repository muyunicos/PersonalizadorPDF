...
- [X] T021 [US4] Tolerancia en `assets/tienda.js`: vista fallida -> `vista.fallida` + ocultada (fotografia final, sin marcos de editor) y `podar()` la quita de la galeria (flechas recalculadas); si NO queda ninguna, la galeria se oculta y aparece el aviso "No hay vista previa disponible: podes comprar igual..." con el carrito HABILITADO (regla V-7: la venta nunca se bloquea); `preview_estado=sin_vista` ya queda persistido por `crear_draft()`/`carrito_promover` (solo pasa a `ok` si se congelo algun webp). Fallo del flujo completo (draft/pool) -> aviso accionable + carrito habilitado. PENDIENTE: soporte de N PDFs por producto en la ficha (hoy el panel es 1 PDF vinculado por producto: `_pmu_pdf_slug`).
...
- [X] T023 [US4] Descarga del PDF final: `item_generar_pdf($order_id,$item_key)` arma el PDF desde `manifest.archivos[]` (indice del pool, nunca adivina por nombre) con `Motor::procesar_pedido()`; idempotente (si ya existe la salida la reusa) y genera desde el staging si aun no se promovio; `descargas_cliente` (`woocommerce_customer_get_downloadable_products`) expone una fila por item en `mi-cuenta/descargas/`; `handle_item_descargar` (`admin_post(_nopriv)_personalizador_pdf_item_descargar`) sirve el PDF autorizando por dueno del pedido, `order_key` (invitado) o admin. PENDIENTE: el re-render CLIENTE desde la pagina de Descargas si falta el pool (hoy el reintento es server-side desde el pool existente, con causa visible).
...
- [X] T026 [US5] Accion "Regenerar" por item (admin): `handle_item_regenerar` (`admin_post_personalizador_pdf_item_regenerar`, `seguridad()` + nonce): borra la salida previa y la rearma desde el pool vigente con `item_generar_pdf()` (idempotente, sin duplicar archivos); errores con causa via redirect. PENDIENTE (mejora): re-render del pool en el navegador del admin (RenderCore) cuando falten PNG; hoy regenera solo con el pool existente.
